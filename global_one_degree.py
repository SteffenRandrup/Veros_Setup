...
DATA_FILES = veros.tools.get_assets("global_1deg", os.path.join(BASE_PATH, "assets_1deg.yml"))
...
class GlobalOneDegree(veros.Veros):
    ...
    def set_initial_conditions(self):
        ...
        self.phytoplankton[...] = 0.14 / (2 ** (self.zt / 100)) * self.maskT
        self.zooplankton[...] = 0.014 / (2 ** (self.zt / 100)) * self.maskT
        self.detritus[...] = 1e-4 * self.maskT
        self.po4[...] = 29.5 #/ 10 #/ 100 #/ (2 ** (np.arange(self.nz))[::-1])
        self.po4[..., -1] = 5.0 #/ 50 #/ 100
        self.po4[...] *= self.maskT
        ...
        if self.enable_nitrogen:
            ...
            self.no3[...] = self.po4[...]
            self.don[...] = 20 * self.maskT
            self.dop[...] = 20 * self.maskT
        ...
        if self.enable_calcifiers:
            ...
            self.caco3[...] = 0.01 * self.maskT
            self.dic[...] = 1 * self.maskT
            self.alkalinity[...] = 1 * self.maskT
    ...
    def set_diagnostics(self):
        self.diagnostics["npzd"].output_frequency = 1 * 86400
        self.diagnostics["npzd"].show_npzd_graph = False
        self.diagnostics["snapshot"].output_frequency = self.runlen // 100
